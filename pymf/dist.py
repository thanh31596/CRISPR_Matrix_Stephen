...
def abs_cosine_distance(d, vec, weighted=False):
    if scipy.sparse.issparse(d):
        ...
        a = np.sqrt(np.array(d.multiply(d).sum(
            axis=0), dtype=np.float32).reshape(-1))
        b = np.sqrt(np.array(vec.multiply(vec).sum(
            axis=0), dtype=np.float32).reshape(-1))
    else:
...
def pdist(A, B, metric='l2'):
    ...
    if A.shape[1] <= B.shape[1]:
        for aidx in range(A.shape[1]):
            if metric == 'l2':
                d[aidx:aidx+1,
                    :] = l2_distance(B[:, :], A[:, aidx:aidx+1]).reshape((1, -1))
            if metric == 'l1':
                d[aidx:aidx+1,
                    :] = l1_distance(B[:, :], A[:, aidx:aidx+1]).reshape((1, -1))
    else:
        for bidx in range(B.shape[1]):
            if metric == 'l2':
                d[:, bidx:bidx +
                    1] = l2_distance(A[:, :], B[:, bidx:bidx+1]).reshape((-1, 1))
            if metric == 'l1':
                d[:, bidx:bidx +
                    1] = l1_distance(A[:, :], B[:, bidx:bidx+1]).reshape((-1, 1))
