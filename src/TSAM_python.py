...
def viterbi(seq, logTR, logE):
    ...
    for count in range(0, L):
        ...
        for state in range(0, numStates):
            bestVal = float('-inf')
            bestPTR = 0
            for inner in range(0, numStates):
                val = vOld[inner][0]
                if val > bestVal:
                    bestVal = val
                    bestPTR = inner
...
def cross_validation_cls_libsvm(train_seqs, Importance, featureAll, labels, splits, C, G, percent, au, ad):
    ...
    for run in range(0, runNum):
        ...
        for spl in range(0, split_num):
            ...
            for i in range(0, len(p_label_a)):
...
def cross_validation_ens(splits, scores, pre_score1, pre_score2, corr_type, k):
    ...
    for i in range(0, len(pre_score1)):
...
def phmm_scores(test_seqs, TRs, ETs):
    ...
    for i in range(0, len(test_seqs[:, 0])):
        Tr1_p = TRs['Tr1_p']
        Tr1_n = TRs['Tr1_n']
        Tr2_p = TRs['Tr2_p']
        Tr2_n = TRs['Tr2_n']
...
def test_final_models(data_index, pretype, featype, phmmtype):
    if pretype == 1:
        ...
        test_seq_20 = []
        test_seq_30 = []
        for i in range(0, len(test_Seqs)):
...
# predict sgRNAs cutting at exon only
    ...
    for i in range(0, len(score_index)):
        score_index[i,0]=i
    ...
    with open(file_path, 'w') as csv_file:
        ...
        writer.writeheader()
        for i in range(0, len(spacers_20)):
            ind = int(out[i][3])                    
            writer.writerow(
